...
$totalDist5 = $totalDist3 = $totalDist2 = 0;
     ...
     if ($lat < $BB{"lly"} || $lat > $BB{"ury"} || $lng < $BB{"llx"} || $lng > $BB{"urx"} ) {##
        ...
        if ($latPrev > 54.0 ) {## first $latPrev is NUL
           ...
           if ($dist > 0) {
             ...
             ###print STDERR "AVESLOPE: $aveSlope $dist $altDiff // '$ltg1' '$ltg2' $StationsEE{$ltg2} - $StationsEE{$ltg1}\n";

             if ($aveSlope > 5 ) { $totalDist5 += $dist ; }
             if ($aveSlope > 3 ) { $totalDist3 += $dist ; }
             if ($aveSlope > 2 ) { $totalDist2 += $dist ; }
           } else { print STDERR "****** ZERO $distance ($lat $lng // $latPrev $lngPrev)\n"}
...
print LOG "\n## SECTIONS SUMMARY -------------------------------------------\n";
print LOG "|   Lp | Start-Finish-Coordinates                        |   Num | Slope | Length |  C--C  |\n";

print KML kml_header("Sections for month $monthN");

for my $s (sort {$VisitedSections{$b} <=> $VisitedSections{$a} } keys %VisitedSections) { 
 $path++;
 if ($VisitedSections{$s} < 30) { last } ### tylko odcinki przejechane co najmniej 1x w miesiącu
 my ($lt1, $lg1, $lt2, $lg2) = split / /, $s;
 my $ltg1 = "$lt1 $lg1"; my $ltg2 = "$lt2 $lg2";

 $altDiff = $StationsEE{"$ltg2"} - $StationsEE{"$ltg1"};
 $dist = $geo->distance( "meter", $lg2, $lt2 => $lg1, $lt1 );
 if ($dist == 0) {$dist = 0.0001} ### division-by-zero
 printf LOG "| %4i | %s | %5i | %5.1f | %6.3f | %s |\n",  $path, $s, 
      $VisitedSections{$s}, $altDiff/$dist*100,   $dist/$kilometer, "$StationsC{$ltg1}--$StationsC{$ltg2}";

 $kml_p_name = sprintf "Section${path}_$VisitedSections{$s}(%.3f)", $dist/$kilometer;

 if ($VisitedSections{$s} < 60) { last } ### do KMLa tylko odcinki przejechane co najmniej 2x w miesiącu
...
### ### ###
printf STDERR "DistTotal: %.1f Dist5/3/2: %.1f %.1f %.1f (%.2f %.2f %.2f)\n", 
   $distGrandTotal/$kilometer, $totalDist5/$kilometer, $totalDist3/$kilometer, $totalDist2/$kilometer,
   $totalDist5/$distGrandTotal*100, $totalDist3/$distGrandTotal*100, $totalDist2/$distGrandTotal*100;
printf LOG "DistTotal: %.1f Dist5/3/2: %.1f %.1f %.1f (%.2f %.2f %.2f)\n", 
   $distGrandTotal/$kilometer, $totalDist5/$kilometer, $totalDist3/$kilometer, $totalDist2/$kilometer,
   $totalDist5/$distGrandTotal*100, $totalDist3/$distGrandTotal*100, $totalDist2/$distGrandTotal*100;
